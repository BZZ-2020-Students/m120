...
public class GamePanel extends JPanel implements ActionListener {
	...
	static final int UNIT_SIZE = 25; // declarate the size of the items in this game
	...
	public void paintComponent(Graphics g) { // method with parameter
	...
	public void draw(Graphics g) {
		if (running) {
			...
			for (int i = 0; i < bodyParts; i++) {
				if (i == 0) {
					g.setColor((new Color(0, 135, 226)));
	...
	public void move() {
		for (int i = bodyParts; i > 0; i--) {
			x[i] = x[i - 1]; // shifting the coordinates in this array
			y[i] = y[i - 1];
		}
